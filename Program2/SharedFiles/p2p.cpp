...
void p2p(string registry_location, uint16_t registry_port, uint32_t uid, int server);
...
int main(int argc, char* argv[]) {
    ...
    uint16_t registry_port = stoi(argv[2]);
    uint32_t uid = stoi(argv[3]);

    //verify valid port # was selected
    if(registry_port <= 2000) {
    ...
    int server;
    string registry_port_str = to_string(registry_port);
    ...
    //run p2p
    p2p(registry_location, registry_port, uid, server);
    close(server);
...
void p2p_search(int server, uint8_t action) {
    ...
    uint32_t peer_ip = ntohl(*reinterpret_cast<uint32_t*>(response + 4));
    ...
    if(peer_id == 0 && peer_ip == 0 && peer_port == 0) { //search response is all 0's, file not in registry
        ...
        cout << "File found at\n";
        cout << "\tPeer " << peer_id << "\n";
        cout << "\t" << inet_ntoa(*reinterpret_cast<in_addr*>(&peer_ip)) << ":" << peer_port << "\n";
    }
}
